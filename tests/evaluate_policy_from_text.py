...
def get_sentences_from_txt(filepath):
    '''
    Clean-up raw text from file
...
def main(url='http://www.seriously.com/privacy-notice/', model_name='reduced_feats'):
    ...
    #path_to_text = '../data/validation/signal.txt'
    path_to_text = '../data/validation/amazon.txt'
    all_sentences = get_sentences_from_txt(path_to_text)

    # load models
    base_path = '/Users/Khaldoon/Dropbox/insight/inphormed/'
    #base_path = 'inphormed/static/'
        ...
        if practiced_policies[policy_indices[pol]] > 0:
            ...
            for res_array in y_oos_pred_numeric:
                if res_array[policy_indices[pol]] > 0:
                    sentences[pol_name].append(all_sentences[ind])
                ind +=1
