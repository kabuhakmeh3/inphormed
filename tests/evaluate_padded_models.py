...
def main():
    ...
    with open('../reports/padded_tfidf_model_training.csv', 'w') as f:
        f.write(header_line)
    ...
    for policy_file in data_files:
        ...
        if len(df) >= 100:
            ...
            with open('../reports/padded_tfidf_model_training.csv', 'a') as f:
                f.write(line_to_write)
        else:
            print('Not enough training samples for '+policy_file[:-4])
            line_to_write = policy_file[:-4]+'\n'
            with open('../reports/padded_tfidf_untrained_models.txt', 'a') as u:
